// Decompiled by DJ v3.12.12.96 Copyright 2011 Atanas Neshkov  Date: 27.08.2013 11:33:12
...
public class Tochki extends Applet
    ...
    public boolean lining() {
        ...
        left_border = step_1_of_2 + 1;
        top_border = step_1_of_2 + 1;
        right_border = board_width - (step_1_of_2 + 1);
        bottom_border = board_height - (step_1_of_2 + 1);
    ...
    public void set_click(int click_mouse_x, int click_mouse_y, int x, int y, int xCoords[], int yCoords[]) {
        if (active == 0) {
            int posY = -1;
            int posX = -1;
            for (int lineIterator = 0; lineIterator < vert_lines; lineIterator++)
                if (x > xCoords[lineIterator] - (step_1_of_2 - 2) && x < xCoords[lineIterator] + step_1_of_2 + 3) {
                    this.click_mouse_x = xCoords[lineIterator];
                    posX = lineIterator;
                }

            for (int lineIterator = 0; lineIterator < gor_lines; lineIterator++)
                if (y > yCoords[lineIterator] - (step_1_of_2 - 2) && y < yCoords[lineIterator] + step_1_of_2 + 3) {
                    this.click_mouse_y = yCoords[lineIterator];
                    posY = lineIterator;
                }

            if (posY != -1 || posX != -1) {
                if (mass_board[posY][posX] == f && dot_condition[posY][posX] == f) {
                    mass_board[posY][posX] = r;
                    dot_condition[posY][posX] = r;
                    for (; OLD_last_dot_x != last_dot_x; OLD_last_dot_x = last_dot_x) ;
                    for (; OLD_last_dot_y != last_dot_y; OLD_last_dot_y = last_dot_y) ;
                    last_dot_str = r;
                    relative_last_x = posX;
                    last_dot_x = x_board[posX];
                    last_dot_y = y_board[posY];
                    relative_last_y = posY;
                    int i2 = 1;
                    active = i2;
                    return;
                }
                if (dot_condition[posY][posX] == n) {
            ...
            for (int j1 = 1; j1 < gor_lines - 1; j1++) {
                ...
                for (int l1 = 1; l1 < vert_lines - 1; l1++) {
                    ...
                    if (ai[k1][i2] == i && dot_condition[k1][i2] == f)
                        if (dot_condition[k1][i2] != n) dot_condition[k1][i2] = n;
                    if (ai[k1][i2] == i && ai[k1 - 1][i2] != i && ai[k1][i2 - 1] != i) {
                        ...
                        if (dot_condition[k1][i2] == f)
                            if (dot_condition[k1][i2] != n) dot_condition[k1][i2] = n;
                        if (ai[k1 - 1][i2 - 1] != i) {
                            if (dot_condition[k1][i2 - 1] != c1) dot_condition[k1][i2 - 1] = c1;
                            if (dot_condition[k1 - 1][i2] != c1) dot_condition[k1 - 1][i2] = c1;
                            if (poligons[j][0][i1] != x_board[i2]) poligons[j][0][i1] = x_board[i2];
                            if (poligons[j][1][i1] != y_board[k1 - 1]) poligons[j][1][i1] = y_board[k1 - 1];
                            if (poligons[j][0][i1 + 1] != x_board[i2 - 1]) poligons[j][0][i1 + 1] = x_board[i2 - 1];
                            if (poligons[j][1][i1 + 1] != y_board[k1]) poligons[j][1][i1 + 1] = y_board[k1];
                            i1 += 2;
                        }
                    }
                    if (ai[k1][i2] == i && ai[k1 - 1][i2] != i && ai[k1][i2 + 1] != i) {
                    ...
                    if (ai[k1][i2] == i && ai[k1][i2 - 1] != i && ai[k1 - 1][i2 - 1] != i && ai[k1 - 1][i2] == i) {
                        ...
                        if (dot_condition[k1][i2] == f)
                            if (dot_condition[k1][i2] != n) dot_condition[k1][i2] = n;
                        if (dot_condition[k1][i2 - 1] != c1) dot_condition[k1][i2 - 1] = c1;
                        if (dot_condition[k1 - 1][i2 - 1] != c1) dot_condition[k1 - 1][i2 - 1] = c1;
                        if (poligons[j][0][i1] != x_board[i2 - 1]) poligons[j][0][i1] = x_board[i2 - 1];
                        if (poligons[j][1][i1] != y_board[k1]) poligons[j][1][i1] = y_board[k1];
                        if (poligons[j][0][i1 + 1] != x_board[i2 - 1]) poligons[j][0][i1 + 1] = x_board[i2 - 1];
                        if (poligons[j][1][i1 + 1] != y_board[k1 - 1]) poligons[j][1][i1 + 1] = y_board[k1 - 1];
                        i1 += 2;
                    }
                    if (ai[k1][i2] == i && ai[k1][i2 - 1] != i && ai[k1 + 1][i2 - 1] != i && ai[k1 + 1][i2] == i) {
                        if (dot_condition[k1][i2] == c)
    ...
    public boolean mouseDown(Event event, int x, int y) {
        if (active == 0) {
            if (x < right_border && y < bottom_border && x > left_border && y > top_border)
                set_click(click_mouse_x, click_mouse_y, x, y, x_board, y_board);
            if (x < board_width - (step_1_of_2 - 7) && y < board_height - (step_1_of_2 - 7) && x > step_1_of_2 - 7 && y > step_1_of_2 - 7) {
                int k = 1;
                user_done = k;
                int l = 0;
                restart = l;
